//project euler problem number 8
//https://projecteuler.net/problem=8

class Program
{
    static void Main(string[] args)
    {
        Stopwatch stopwatch = Stopwatch.StartNew();

        const string number = @"";

        List<char> charList = [..number.ToCharArray()];

        int i = 0;
        List<long> multiplyList = [];
        bool flag = false;
        int listLength = charList.Count;

        while (i < listLength)
        {
            long multiply = 1;

            if (i + 13 >= listLength)
            {
                break;
            }

            for (int j = 0; j < 13; j++)
            {
                if ((int)char.GetNumericValue(charList[i + j]) == 0)
                {
                    i += j + 1;

                    flag = true;

                    break;
                }
            }

            if (flag)
            {
                flag = false;

                continue;
            }

            for (int j = 0; j < 13; j++)
            {
                multiply *= (int)char.GetNumericValue(charList[i + j]);
            }

            multiplyList.Add(multiply);

            i += 1;
        }

        Console.WriteLine(multiplyList.Max());

        stopwatch.Stop();

        long elapsedMilliseconds = stopwatch.ElapsedMilliseconds;

        Console.WriteLine($"Program worked in {elapsedMilliseconds} ms.");
    }
}